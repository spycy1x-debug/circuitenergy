...
const BUNDLES: Bundle[] = [
  { id: "b1", name: "1 CinchWrap", qty: 1, price: 39.0, compareAt: 59.0 },
  { id: "b2", name: "2 CinchWraps", qty: 2, price: 68.0, compareAt: 118.0, tag: "MOST POPULAR" },
  { id: "b3", name: "3 CinchWraps", qty: 3, price: 89.0, compareAt: 177.0 },
];
  ...
  return (
    ...
    >
      <div>
        <div
          style={sans}
          className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[color:var(--cw-brand)]"
        >
          {label}
        </div>
...
const FAQS = [
  ...
  },
  {
    ...
    a: "One CinchWrap adjusts across the full XS–3XL range because you set the tension yourself. Use the size guide to confirm your band length, and if you are between sizes, size up.",
  },
  {
    ...
    <div className="border-t border-[color:var(--cw-line)]">
      {FAQS.map((f, i) => (
        <div key={f.q} className="border-b border-[color:var(--cw-line)]">
          ...
          >
            <span style={serif} className="text-[19px] leading-tight text-[color:var(--cw-ink)]">
              {f.q}
            </span>
            <span
              aria-hidden
              className="shrink-0 text-xl leading-none text-[color:var(--cw-brand)]"
            >
              {open === i ? "–" : "+"}
            </span>
          </button>
          {open === i && (
            <p style={sans} className="pb-6 pr-8 text-[14px] leading-7 text-[color:var(--cw-muted)]">
              {f.a}
            </p>
          )}
        </div>
...
function BuyBox() {
  ...
  return (
    <div>
      ...
      <h2 style={serif} className="mt-3 text-[34px] leading-[1.05] text-[color:var(--cw-ink)] md:text-[42px]">
        CinchWrap™ Adjustable Waist Wrap
      </h2>
            ...
            <button
              ...
              className={`relative flex w-full items-center gap-4 rounded-2xl border bg-[color:var(--cw-bg)] px-4 py-4 text-left transition-colors ${
                on
                  ...
                  : "border-[color:var(--cw-line)] hover:border-[color:var(--cw-muted)]"
              }`}
            >
              {b.tag && (
                <span
              ...
              <span className="min-w-0 flex-1">
                <span style={serif} className="block text-[18px] text-[color:var(--cw-ink)]">
                  {b.name}
                </span>
...
export function CinchWrap() {
  return (
    <div style={{ ...VARS, ...sans }} className="min-h-screen bg-[color:var(--cw-bg)] text-[color:var(--cw-ink)]">
      {/* 1. Announcement */}
      ...
      {/* Nav */}
      <header className="border-b border-[color:var(--cw-line)]">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 md:px-8">
          <span style={serif} className="text-[20px] tracking-tight">
            CinchWrap<sup className="text-[9px]">™</sup>
          </span>
          <a
            href="#shop"
            className="rounded-full border border-[color:var(--cw-brand)] px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-[color:var(--cw-brand)]"
          >
            Shop
          </a>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 px-5 py-14 md:grid-cols-2 md:gap-16 md:px-8 md:py-24">
        <div>
          <Label>CinchWrap™ · One size, wrapped to you</Label>
          <h1 style={serif} className="mt-5 text-[42px] leading-[1.02] tracking-tight md:text-[64px]">
            The waist wrap that actually stays put.
          </h1>
          <p className="mt-5 max-w-md text-[16px] leading-8 text-[color:var(--cw-muted)]">
            One adjustable band wraps to your exact waist, takes inches off your silhouette on contact, and
            stays comfortable from morning coffee to midnight.
          </p>
          <a
            href="#shop"
            className="mt-8 inline-block rounded-full bg-[color:var(--cw-brand)] px-8 py-4 text-[12px] font-semibold uppercase tracking-[0.2em] text-white transition-opacity hover:opacity-90"
          >
            Shop CinchWrap — $39
          </a>
        </div>
        <Placeholder ratio="4 / 5" note="Hero: full-body shot, wrap worn under a fitted slip dress." />
      </section>
          ...
          {STEPS.map((s) => (
            <div key={s.n}>
              ...
              <div className="mt-5 flex items-baseline gap-3">
                <span style={serif} className="text-[20px] text-[color:var(--cw-brand)]">
                  {s.n}
                </span>
                <h3 style={serif} className="text-[22px] leading-tight">
                  {s.t}
                </h3>
              </div>
      ...
      <section className="border-y border-[color:var(--cw-line)] bg-[color:var(--cw-surface)]/50">
        <div className="mx-auto max-w-4xl px-5 py-16 md:px-8 md:py-24">
          ...
          <h2 style={serif} className="mt-4 text-[34px] leading-[1.08] md:text-[46px]">
            CinchWrap™ vs. traditional waist trainers.
          </h2>
          <div className="mt-10 overflow-hidden rounded-2xl border border-[color:var(--cw-line)] bg-[color:var(--cw-bg)]">
            ...
            {ROWS.map((r) => (
              <div
                key={r.k}
                className="grid grid-cols-[0.8fr_1.1fr_1fr] border-b border-[color:var(--cw-line)] last:border-0"
              >
                ...
                <div className="px-3 py-4 text-[13px] leading-6 text-[color:var(--cw-muted)] md:px-5">
                  {r.theirs}
                </div>
              </div>
            ))}
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-14 md:grid-cols-3 md:px-8">
          <div>
            <span style={serif} className="text-[22px]">
              CinchWrap<sup className="text-[10px]">™</sup>
            </span>
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-[color:var(--cw-brand)]">
              Contact
            </p>
            <p className="mt-3 text-[13px] leading-7 text-[color:var(--cw-muted)]">
              support@cinchwrap.com
              <br />
              Mon–Fri, 9am–5pm ET
            </p>
          </div>
          <div>
            ...
            <ul className="mt-3 space-y-2 text-[13px] text-[color:var(--cw-muted)]">
              <li><a href="/shipping" className="hover:text-[color:var(--cw-ink)]">Shipping</a></li>
              <li><a href="/refund" className="hover:text-[color:var(--cw-ink)]">Returns &amp; refunds</a></li>
              <li><a href="/privacy" className="hover:text-[color:var(--cw-ink)]">Privacy</a></li>
              <li><a href="/terms" className="hover:text-[color:var(--cw-ink)]">Terms</a></li>
            </ul>
          </div>
        </div>
        <div className="border-t border-[color:var(--cw-line)]">
          <p className="mx-auto max-w-6xl px-5 py-5 text-[11px] text-[color:var(--cw-muted)] md:px-8">
            © {new Date().getFullYear()} CinchWrap™. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
