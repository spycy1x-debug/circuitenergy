      ...
      <section className="relative isolate overflow-hidden bg-[oklch(0.15_0.03_245)] text-white">
        ...
        <div className="container-x py-16 md:py-20 max-w-3xl text-center mx-auto">
          ...
          <h1 className="mt-6 text-5xl md:text-7xl leading-[0.95] text-white">
            Your <em style={serif} className="italic font-normal text-[oklch(0.85_0.15_70)]">Cart</em>
          </h1>
          <p className="mt-5 text-lg md:text-xl text-white/80">
            {count === 0 ? (
              <>A blank slate. Let's build your <em style={serif} className="italic text-white">stack</em>.</>
            ) : (
              <>One step closer to feeling like <em style={serif} className="italic text-white">yourself</em> again.</>
            )}
          </p>
        </div>
      </section>
        ...
        {count === 0 ? (
            ...
            <div className="relative overflow-hidden rounded-3xl border border-border bg-white shadow-md p-10 md:p-14 text-center">
              ...
              <div className="relative">
                ...
                <h2 className="mt-6 text-3xl md:text-4xl">
                  Your cart is <em style={serif} className="italic font-normal">empty</em>
                </h2>
                ...
                <div className="mt-8 flex flex-wrap justify-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-3.5 w-3.5 text-success"/>60-day guarantee</span>
                  <span className="inline-flex items-center gap-1.5"><Truck className="h-3.5 w-3.5 text-primary"/>Free shipping over $75</span>
                  <span className="inline-flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5 text-ink"/>Clean ingredients</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-10 lg:grid-cols-[1.6fr_1fr] items-start">
            ...
            <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-border bg-secondary/60 p-5">
                  ...
                  <div className="mt-3 h-2 w-full rounded-full bg-border overflow-hidden">
                    <div
                      ...
                      style={{ width: `${progress}%`, background: "linear-gradient(90deg, oklch(0.65 0.17 290), oklch(0.75 0.18 55))" }}
                    />
                  </div>
                </div>
              )}
              ...
              {items.map(it => (
                <div key={it.id} className="group relative rounded-2xl border border-border bg-white p-5 flex gap-5 items-center transition-shadow hover:shadow-md">
                  <div
                    className="h-28 w-28 rounded-xl flex items-center justify-center shrink-0 overflow-hidden"
                    style={{ background: "linear-gradient(160deg, oklch(0.98 0.02 290), white)", boxShadow: "inset 0 0 0 1px oklch(0.65 0.17 290 / 0.2)" }}
                  >
                    <img src={it.image} alt={it.name} className="max-h-24 object-contain transition-transform duration-500 group-hover:scale-105"/>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-3 inline-flex items-center border border-border rounded-md bg-white">
                      <button aria-label="Decrease" onClick={()=>cart.setQty(it.id, it.qty-1)} className="h-9 w-9 flex items-center justify-center hover:bg-secondary transition-colors"><Minus className="h-3.5 w-3.5"/></button>
                      <span className="w-10 text-center text-sm font-semibold">{it.qty}</span>
                      <button aria-label="Increase" onClick={()=>cart.setQty(it.id, it.qty+1)} className="h-9 w-9 flex items-center justify-center hover:bg-secondary transition-colors"><Plus className="h-3.5 w-3.5"/></button>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-display font-bold text-ink text-lg">${(it.price*it.qty).toFixed(2)}</div>
                    <button onClick={()=>cart.remove(it.id)} aria-label="Remove" className="mt-2 inline-flex items-center justify-center h-8 w-8 rounded-full text-muted-foreground hover:text-destructive hover:bg-secondary transition-colors"><X className="h-4 w-4"/></button>
                  </div>
                </div>
              ))}
              ...
              <div className="relative overflow-hidden rounded-2xl border border-border bg-white shadow-md p-6">
                ...
                <div className="relative">
                  <h2 className="text-2xl">
                    Order <em style={serif} className="italic font-normal">Summary</em>
                  </h2>
                  <div className="mt-5 space-y-3 text-sm">
                    <Row label="Subtotal" value={`$${subtotal.toFixed(2)}`}/>
                    <Row label={shipping===0?"Shipping":"Shipping"} value={shipping===0?"FREE":`$${shipping.toFixed(2)}`}/>
                  ...
                  <div className="mt-5 space-y-2 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-2"><Lock className="h-4 w-4 text-ink"/>SSL encrypted checkout</div>
                  </div>
                </div>
              </div>
            </div>
